...
const PLUGIN_ID = 'cimanow';
function log(msg, data) { try { console.log(`[${PLUGIN_ID}] ${msg}`, data || ''); } catch (_) {} }

const baseUrl = typeof manifest !== 'undefined' ? manifest.baseUrl : 'https://cimanow.cc';
...
async function getHome(cb) {
    ...
    try {
        ...
        $(".BlockItem, .post-item").each((i, el) => {
            ...
            const title = $(el).find(".Title, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
