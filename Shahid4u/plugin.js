...
const PLUGIN_ID = 'shahid4u';
function log(msg, data) { try { console.log(`[${PLUGIN_ID}] ${msg}`, data || ''); } catch (_) {} }

const baseUrl = typeof manifest !== 'undefined' ? manifest.baseUrl : 'https://shahid4u.com';
...
async function getHome(cb) {
    ...
    try {
        ...
        const items = [];
        $(".post-item, .entry, .movie-card").each((i, el) => {
            const a = $(el).find("a").first();
            if (!a.length) return;
            const href = a.attr("href") || "";
            if (!href || href === baseUrl || href === baseUrl + "/") return;
            const title = $(el).find(".entry-title a, h2 a, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
