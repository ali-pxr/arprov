...
async function getHome(page, cb) {
    ...
    for (const [name, path] of Object.entries(cats)) {
        try {
            ...
            const src = posts.length > 0 ? posts : matchAll(html, /<div[^>]*class="[^"]*(?:post-item|entry|anime-card|poster)[^"]*"[^>]*>([\s\S]*?)<\/div>\s*(?:<\/div>)?/gi);
...
async function load(url, cb) {
    try {
        ...
        let title = (html.match(/<h1[^>]*>([^<]*)/i) || [])[1] || "Unknown";
        const posterUrl = (html.match(/data-(?:src|image)="([^"]+)"/) || [])[1] || (html.match(/property="og:image"\s+content="([^"]+)"/) || [])[1] || '';
        const plot = (html.match(/class="[^"]*(?:story|desc|content)[^"]*"[^>]*>([\s\S]*?)<\/div>/i) || [])[1] || '';

        const episodes = [];
        const epLinks = matchAll(html, /href="([^"]*(?:episode|الحلقة)[^"]*)"/gi);
        const seen = new Set();
