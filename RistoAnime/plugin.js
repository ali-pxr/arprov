...
const PLUGIN_ID = 'ristoanime';
function log(msg, data) { try { console.log(`[${PLUGIN_ID}] ${msg}`, data || ''); } catch (_) {} }

const baseUrl = typeof manifest !== 'undefined' ? manifest.baseUrl : 'https://ristoanime.com';
...
async function getHome(cb) {
    ...
    try {
        ...
        $(".anime-card, .post-item").each((i, el) => {
            ...
            const title = $(el).find("h3 a, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
...
async function search(query, cb) {
    try {
        ...
        $(".anime-card, .post-item").each((i, el) => {
            ...
            const href = a.attr("href") || "";
            if (!href) return;
            const title = $(el).find("h3, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
...
async function loadStreams(url, cb) {
    try {
        ...
        $("iframe[src], .server-btn").each((i, el) => tryExt($(el).attr("src")));
        $("iframe[src], .server-btn").each((i, el) => tryExt($(el).attr("data-src")));
