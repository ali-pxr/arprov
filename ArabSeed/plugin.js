...
const PLUGIN_ID = 'arabseed';
function log(msg, data) { try { console.log(`[${PLUGIN_ID}] ${msg}`, data || ''); } catch (_) {} }

const baseUrl = typeof manifest !== 'undefined' ? manifest.baseUrl : 'https://arabseed.co';
...
async function getHome(cb) {
    ...
    try {
        ...
        $(".post-item, .Entry").each((i, el) => {
            ...
            const title = $(el).find(".post-title a, .entry-title a, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
            const poster = $(el).find("img").first().attr("data-src") || $(el).find("img").first().attr("data-image") || $(el).find("img").first().attr("src") || "";
            const type = href.includes("مسلسل") || href.includes("series") ? "series" : href.includes("انمي") || href.includes("anime") ? "anime" : "movie";
    ...
    try {
        const html = await httpFetch(baseUrl + "/category/series/");
        const $ = cheerio.load(html);
        const items = [];
        $(".post-item, .Entry").each((i, el) => {
            const a = $(el).find("a").first();
    ...
    try {
        const html = await httpFetch(baseUrl + "/category/anime/");
        const $ = cheerio.load(html);
        const items = [];
        $(".post-item, .Entry").each((i, el) => {
            const a = $(el).find("a").first();
        ...
        if (items.length) data["أنمي"] = items;
    } catch (e) { log("getHome error", e); }
...
async function search(query, cb) {
    try {
        ...
        $(".post-item, .Entry").each((i, el) => {
            ...
            const href = a.attr("href") || "";
            if (!href) return;
            const title = $(el).find(".post-title, .entry-title, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
...
async function load(url, cb) {
    try {
        ...
        const $ = cheerio.load(html);
        const title = $("h1, .post-title").first().text().trim() || $("title").text() || "Unknown";
        const posterUrl = $(".post-img img, img.wp-post-image").first().attr("data-src") || $(".post-img img, img.wp-post-image").first().attr("data-image") || $(".post-img img, img.wp-post-image").first().attr("src") || "";
        const plot = $(".post-content, .entry-content p").first().text().trim() || "";
        const yearMatch = $("body").text().match(/(19|20)\d{2}/);
        const year = yearMatch ? parseInt(yearMatch[0]) : undefined;
        const isSeries = $("ul.Episodes, ul.insert_ep, div.Episodes, .episodes-list a, .season-episodes a, a[href*='episode']").length > 0;
        const episodes = [];
        $(".episodes-list a, .season-episodes a, a[href*='episode']").each((i, el) => {
            const a = $(el).find("a").first();
            if (!a.length) return;
            const href = a.attr("href") || "";
            if (!href) return;
...
async function loadStreams(url, cb) {
    try {
        ...
        $("iframe[src], .servers-list li, .download-links a").each((i, el) => tryExt($(el).attr("src")));
        $("iframe[src], .servers-list li, .download-links a").each((i, el) => tryExt($(el).attr("data-src")));
        $(".servers-list li, li[data-url]").each((i, el) => {
