...
const PLUGIN_ID = 'animeiat';
function log(msg, data) { try { console.log(`[${PLUGIN_ID}] ${msg}`, data || ''); } catch (_) {} }

const baseUrl = typeof manifest !== 'undefined' ? manifest.baseUrl : 'https://animeiat.tv';
...
async function getHome(cb) {
    const data = {};
    try {
        const html = await httpFetch(baseUrl + "/anime-list/");
        const $ = cheerio.load(html);
        const items = [];
        $(".anime-card, .poster").each((i, el) => {
            const a = $(el).find("a").first();
            if (!a.length) return;
            const href = a.attr("href") || "";
            if (!href || href === baseUrl || href === baseUrl + "/") return;
            const title = $(el).find("h3, .title, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
        ...
        if (items.length) data["أحدث الأنمي"] = items;
    } catch (e) { log("getHome error", e); }
    try {
        const html = await httpFetch(baseUrl + "/anime-movies/");
        const $ = cheerio.load(html);
        const items = [];
        $(".anime-card, .poster").each((i, el) => {
            const a = $(el).find("a").first();
        ...
        if (items.length) data["أفلام الأنمي"] = items;
    } catch (e) { log("getHome error", e); }
...
async function search(query, cb) {
    try {
        const html = await httpFetch(`{baseUrl}/search?q={{QUERY}}`.replace("{QUERY}", encodeURIComponent(query)));
        const $ = cheerio.load(html);
        const items = [];
        $(".anime-card, .poster").each((i, el) => {
            const a = $(el).find("a").first();
...
async function load(url, cb) {
    try {
        ...
        const $ = cheerio.load(html);
        const title = $("h1, .anime-title").first().text().trim() || $("title").text() || "Unknown";
        const posterUrl = $(".poster img, img").first().attr("data-src") || $(".poster img, img").first().attr("data-image") || $(".poster img, img").first().attr("src") || "";
        const plot = $(".anime-desc, .story, .description").first().text().trim() || "";
        const yearMatch = $("body").text().match(/(19|20)\d{2}/);
        const year = yearMatch ? parseInt(yearMatch[0]) : undefined;
        const isSeries = $("ul.Episodes, ul.insert_ep, div.Episodes, .episodes-list a, .episode-item, a[href*='episode']").length > 0;
        const episodes = [];
        $(".episodes-list a, .episode-item, a[href*='episode']").each((i, el) => {
            const a = $(el).find("a").first();
            if (!a.length) return;
            const href = a.attr("href") || "";
            if (!href) return;
            const name = $(el).find(".ep-name, a").first().text().trim() || a.text().trim();
...
async function loadStreams(url, cb) {
    try {
        ...
        $("iframe[src], .server-btn, li[data-url]").each((i, el) => tryExt($(el).attr("src")));
        $("iframe[src], .server-btn, li[data-url]").each((i, el) => tryExt($(el).attr("data-src")));
        $(".server-btn, li[data-url]").each((i, el) => {
