...
const PLUGIN_ID = 'faselhd';
function log(msg, data) { try { console.log(`[${PLUGIN_ID}] ${msg}`, data || ''); } catch (_) {} }

const baseUrl = typeof manifest !== 'undefined' ? manifest.baseUrl : 'https://faselhd.co';
...
async function getHome(cb) {
    ...
    try {
        ...
        const items = [];
        $(".post-item, .Entry, .movie-item").each((i, el) => {
            const a = $(el).find("a").first();
            if (!a.length) return;
            const href = a.attr("href") || "";
            if (!href || href === baseUrl || href === baseUrl + "/") return;
            const title = $(el).find(".post-title a, h2 a, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
...
async function search(query, cb) {
    try {
        ...
        $(".post-item, .Entry, .movie-item").each((i, el) => {
            ...
            const href = a.attr("href") || "";
            if (!href) return;
            const title = $(el).find(".post-title, h2, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
...
async function load(url, cb) {
    try {
        ...
        const posterUrl = $("img.wp-post-image, .post-img img").first().attr("data-src") || $("img.wp-post-image, .post-img img").first().attr("data-image") || $("img.wp-post-image, .post-img img").first().attr("src") || "";
        const plot = $(".post-content p, .story").first().text().trim() || "";
        const yearMatch = $("body").text().match(/(19|20)\d{2}/);
        const year = yearMatch ? parseInt(yearMatch[0]) : undefined;
        const isSeries = $("ul.Episodes, ul.insert_ep, div.Episodes, .episodes-list a, a[href*='episode'], .season-episodes a").length > 0;
        const episodes = [];
        $(".episodes-list a, a[href*='episode'], .season-episodes a").each((i, el) => {
            const a = $(el).find("a").first();
            if (!a.length) return;
            const href = a.attr("href") || "";
            if (!href) return;
