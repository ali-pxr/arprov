...
const PLUGIN_ID = 'gateanime';
function log(msg, data) { try { console.log(`[${PLUGIN_ID}] ${msg}`, data || ''); } catch (_) {} }

const baseUrl = typeof manifest !== 'undefined' ? manifest.baseUrl : 'https://gateanime.com';
...
async function getHome(cb) {
    ...
    try {
        ...
        const items = [];
        $(".anime-card, .post-item").each((i, el) => {
            const a = $(el).find("a").first();
            if (!a.length) return;
            const href = a.attr("href") || "";
            if (!href || href === baseUrl || href === baseUrl + "/") return;
            const title = $(el).find("h3 a, .title a, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
...
async function load(url, cb) {
    try {
        ...
        const plot = $(".anime-desc, .story").first().text().trim() || "";
