...
const PLUGIN_ID = 'cimaclub';
function log(msg, data) { try { console.log(`[${PLUGIN_ID}] ${msg}`, data || ''); } catch (_) {} }

const baseUrl = typeof manifest !== 'undefined' ? manifest.baseUrl : 'https://cimaclub.com';
...
async function getHome(cb) {
    ...
    try {
        ...
        const items = [];
        $(".BlockItem, .post-item, li").each((i, el) => {
            const a = $(el).find("a").first();
            if (!a.length) return;
            const href = a.attr("href") || "";
            if (!href || href === baseUrl || href === baseUrl + "/") return;
            const title = $(el).find(".Title, .post-title a, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
...
async function search(query, cb) {
    try {
        ...
        const $ = cheerio.load(html);
        const items = [];
        $(".BlockItem, .post-item").each((i, el) => {
            const a = $(el).find("a").first();
            if (!a.length) return;
            const href = a.attr("href") || "";
            if (!href) return;
            const title = $(el).find(".Title, .post-title, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
...
async function load(url, cb) {
    try {
        ...
        const $ = cheerio.load(html);
        const title = $("h1, .Title").first().text().trim() || $("title").text() || "Unknown";
        const posterUrl = $("img").first().attr("data-src") || $("img").first().attr("data-image") || $("img").first().attr("src") || "";
        const plot = $(".Story, .story, p").first().text().trim() || "";
        const yearMatch = $("body").text().match(/(19|20)\d{2}/);
        const year = yearMatch ? parseInt(yearMatch[0]) : undefined;
        const isSeries = $("ul.Episodes, ul.insert_ep, div.Episodes, .episodes-list a, ul.Episodes a, a[href*='episode']").length > 0;
        const episodes = [];
        $(".episodes-list a, ul.Episodes a, a[href*='episode']").each((i, el) => {
            const a = $(el).find("a").first();
            if (!a.length) return;
            const href = a.attr("href") || "";
            if (!href) return;
...
async function loadStreams(url, cb) {
    try {
        ...
        $("iframe[src], .serversWatchSide li, .DownloadServers a").each((i, el) => tryExt($(el).attr("src")));
        $("iframe[src], .serversWatchSide li, .DownloadServers a").each((i, el) => tryExt($(el).attr("data-src")));
