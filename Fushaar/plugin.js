...
const PLUGIN_ID = 'fushaar';
function log(msg, data) { try { console.log(`[${PLUGIN_ID}] ${msg}`, data || ''); } catch (_) {} }

const baseUrl = typeof manifest !== 'undefined' ? manifest.baseUrl : 'https://fushaar.com';
...
async function getHome(cb) {
    ...
    try {
        ...
        $(".movie-item, .post-item").each((i, el) => {
            ...
            const title = $(el).find("h2 a, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
    ...
    try {
        const html = await httpFetch(baseUrl + "/category/foreign/");
        const $ = cheerio.load(html);
        const items = [];
        $(".movie-item, .post-item").each((i, el) => {
            const a = $(el).find("a").first();
        ...
        if (items.length) data["أفلام أجنبي"] = items;
    } catch (e) { log("getHome error", e); }
...
async function search(query, cb) {
    try {
        ...
        $(".movie-item, .post-item").each((i, el) => {
            ...
            const href = a.attr("href") || "";
            if (!href) return;
            const title = $(el).find("h2, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
