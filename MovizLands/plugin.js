...
async function getHome(cb) {
    ...
    try {
        ...
        $(".post-item, .Entry").each((i, el) => {
            ...
            const title = $(el).find(".post-title a, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
...
async function search(query, cb) {
    try {
        ...
        $(".post-item, .Entry").each((i, el) => {
            ...
            const href = a.attr("href") || "";
            if (!href) return;
            const title = $(el).find(".post-title, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
...
async function load(url, cb) {
    try {
        ...
        const plot = $(".post-content p").first().text().trim() || "";
