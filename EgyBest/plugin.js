...
const PLUGIN_ID = 'egybest';
function log(msg, data) { try { console.log(`[${PLUGIN_ID}] ${msg}`, data || ''); } catch (_) {} }

const baseUrl = typeof manifest !== 'undefined' ? manifest.baseUrl : 'https://egybest.co';
...
async function getHome(cb) {
    const data = {};
    try {
        const html = await httpFetch(baseUrl + "/movies/");
        const $ = cheerio.load(html);
        const items = [];
        $(".movie, .browse-movie, .result-item").each((i, el) => {
            const a = $(el).find("a").first();
            if (!a.length) return;
            const href = a.attr("href") || "";
            if (!href || href === baseUrl || href === baseUrl + "/") return;
            const title = $(el).find("h2 a, .movie-title, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
            const poster = $(el).find("img").first().attr("data-src") || $(el).find("img").first().attr("data-image") || $(el).find("img").first().attr("src") || "";
            const type = href.includes("series") || href.includes("مسلسل") ? "series" : "movie";
    ...
    try {
        const html = await httpFetch(baseUrl + "/series/");
        const $ = cheerio.load(html);
        const items = [];
        $(".movie, .browse-movie, .result-item").each((i, el) => {
            const a = $(el).find("a").first();
    ...
    try {
        const html = await httpFetch(baseUrl + "/anime/");
        const $ = cheerio.load(html);
        const items = [];
        $(".movie, .browse-movie, .result-item").each((i, el) => {
            const a = $(el).find("a").first();
...
async function search(query, cb) {
    try {
        ...
        $(".movie, .browse-movie, .result-item").each((i, el) => {
            ...
            const href = a.attr("href") || "";
            if (!href) return;
            const title = $(el).find("h2, .movie-title, a").first().text().trim() || a.attr("title") || a.text().trim();
            if (!title) return;
...
async function load(url, cb) {
    try {
        ...
        const $ = cheerio.load(html);
        const title = $("h1, .movie-title").first().text().trim() || $("title").text() || "Unknown";
        const posterUrl = $(".movie-poster img, img").first().attr("data-src") || $(".movie-poster img, img").first().attr("data-image") || $(".movie-poster img, img").first().attr("src") || "";
        const plot = $(".movie-desc, .story").first().text().trim() || "";
